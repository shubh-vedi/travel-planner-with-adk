...
def find_nearby_places_open(query: str, location: str, radius: int = 5000, limit: int = 5) -> str:
    """
    ...
    try:
        ...
        # Parks/beaches/gardens are OSM "way" (polygon) elements tagged with
        # leisure=park or tourism=*, NOT node/amenity — so we must query both.
        ...
        (
          ...
          way["leisure"~"{query}", i](around:{radius},{lat},{lon});
          way["tourism"~"{query}", i](around:{radius},{lat},{lon});
        );
        out center {limit};
        """
